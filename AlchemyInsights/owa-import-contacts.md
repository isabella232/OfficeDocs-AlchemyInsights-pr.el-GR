---
title: '8000019: Πώς να προσθέσετε ή να εισαγάγετε επαφές'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 7eb53224bb3370fc434e520d69910d0c9c23f501
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: el-GR
ms.lasthandoff: 09/14/2020
ms.locfileid: "47694059"
---
# <a name="import-contacts"></a>Εισαγωγή επαφών

Πριν από την εισαγωγή επαφών στο Outlook στο Web, θα πρέπει να τις εξαγάγετε από την άλλη υπηρεσία παροχής ηλεκτρονικού ταχυδρομείου σας ως αρχείο. csv. Στη συνέχεια, ακολουθήστε τις παρακάτω οδηγίες.

1. Στο κάτω μέρος της σελίδας, επιλέξτε <img src='data:image/png;base64,' />
 **Άτομα**.
2. Στη γραμμή εργαλείων, επιλέξτε **Διαχείριση**  >  **επαφών εισαγωγής**.
3. Επιλέξτε **Αναζήτηση**, επιλέξτε το αρχείο CSV και, στη συνέχεια, επιλέξτε **Άνοιγμα**.
4. Επιλέξτε **Εισαγωγή**.

Για να μάθετε περισσότερα, ανατρέξτε [στο θέμα Εισαγωγή επαφών στο Outlook](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web).

