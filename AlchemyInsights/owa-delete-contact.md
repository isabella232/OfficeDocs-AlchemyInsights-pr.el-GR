---
title: Διαγραφή επαφής στο Outlook στο Web
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom:
- "8000012"
- "1997"
ms.openlocfilehash: 4c512d4f3454c8cdf84e7cab61ea10e054f6d897
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: el-GR
ms.lasthandoff: 09/14/2020
ms.locfileid: "47671048"
---
# <a name="delete-a-contact"></a>Διαγραφή επαφής

1. Στην κάτω αριστερή γωνία της σελίδας στο Outlook στο Web, επιλέξτε <img src='data:image/png;base64,' />
 **Άτομα**.
2. Επιλέξτε την επαφή που θέλετε να διαγράψετε και επιλέξτε **Διαγραφή**.

Εάν δεν μπορείτε να επιλέξετε **Διαγραφή**, η επαφή μπορεί να είναι από το Skype ή από έναν συνδεδεμένο λογαριασμό κοινωνικού δικτύου. Για να διαγράψετε την επαφή, μεταβείτε στο Skype ή στο λογαριασμό όπου βρίσκεται η επαφή.