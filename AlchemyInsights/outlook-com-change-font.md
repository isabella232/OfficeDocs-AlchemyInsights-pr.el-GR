---
title: Αλλαγή γραμματοσειράς στο Outlook.com
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "268"
- "1919"
- "8000013"
ms.assetid: 824035f6-90d3-4ea2-a92b-6b73b4ec0076
ms.openlocfilehash: 24827f7f6bc3db41b7e8dc6d22797cfaffc28960
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: el-GR
ms.lasthandoff: 09/15/2020
ms.locfileid: "47768895"
---
# <a name="change-font-or-font-size"></a>Αλλαγή γραμματοσειράς ή μεγέθους γραμματοσειράς

Κατά την ανάγνωση μηνυμάτων ηλεκτρονικού ταχυδρομείου στο Outlook.com, μπορείτε να χρησιμοποιήσετε τη συνάρτηση ζουμ του προγράμματος περιήγησής σας για να μεγεθύνετε ή να μικρύνετε το κείμενο στην οθόνη σας.
  
Για να αλλάξετε τη γραμματοσειρά ή το μέγεθος γραμματοσειράς κατά τη σύνταξη ενός μηνύματος:
  
1. Στο κάτω μέρος του παραθύρου σύνθεσης, επιλέξτε τη **γραμματοσειρά** <img src='data:image/png;base64,' /> ή **μέγεθος γραμματοσειράς** <img src='data:image/png;base64,' /> εικονίδια.
2. Επιλέξτε τη γραμματοσειρά ή το μέγεθος γραμματοσειράς που θέλετε να χρησιμοποιήσετε.

Εάν δεν βλέπετε το μενού επιλογών μορφοποίησης στο κάτω μέρος του παραθύρου σύνθεσης, επιλέξτε τις **επιλογές μορφοποίησης** <img src='data:image/png;base64,' /> εικονίδιο.
  
Για περισσότερες πληροφορίες, ανατρέξτε [στο θέμα Αλλαγή του μεγέθους γραμματοσειράς και της εμφάνισης του μηνύματος ηλεκτρονικού ταχυδρομείου στο Outlook.com](https://support.office.com/article/0b4eb323-23fc-4d5d-adbf-cae14c9c0386?wt.mc_id=Office_Outlook_com_Alchemy).