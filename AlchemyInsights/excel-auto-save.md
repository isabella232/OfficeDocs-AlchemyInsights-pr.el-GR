---
title: Τι είναι η Αυτόματη Αποθήκευση
ms.author: chrsm
author: chrsm
manager: jecon
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "2446"
- "9000610"
ms.openlocfilehash: 63573912b4beb290a0289c53ccd543271c19a76a
ms.sourcegitcommit: 55eff703a17e500681d8fa6a87eb067019ade3cc
ms.translationtype: MT
ms.contentlocale: el-GR
ms.lasthandoff: 04/22/2020
ms.locfileid: "43714222"
---
# <a name="what-is-autosave"></a>Τι είναι η Αυτόματη Αποθήκευση;

Η Αυτόματη Αποθήκευση είναι μια δυνατότητα διαθέσιμη στο Excel, το Word και το PowerPoint για συνδρομητές του Microsoft 365 που αποθηκεύει αυτόματα το αρχείο σας, κάθε λίγα δευτερόλεπτα, καθώς εργάζεστε. 

Για συνήθεις ερωτήσεις, ανατρέξτε στο θέμα [Τι είναι η Αυτόματη Αποθήκευση;](https://support.office.com/article/6d6bd723-ebfd-4e40-b5f6-ae6e8088f7a5)