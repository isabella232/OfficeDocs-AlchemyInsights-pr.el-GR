---
title: Τρόπος απάντησης ή προώθησης μηνυμάτων σε Outlook στο web
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000033"
ms.openlocfilehash: f9d7926c69454c2aafde4c4b8b566eb1ddad66e511e8e089e1383aa4dee4b1ad
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: el-GR
ms.lasthandoff: 08/05/2021
ms.locfileid: "53982697"
---
# <a name="how-to-reply-to-or-forward-messages-in-outlook-on-the-web"></a>Τρόπος απάντησης ή προώθησης μηνυμάτων σε Outlook στο web

Στην επάνω δεξιά γωνία του παραθύρου μηνύματος, επιλέξτε **"Απάντηση"** <img src='data:image/png;base64,' />, **Απάντηση σε όλους** <img src='data:image/png;base64,' />ή **"Προώθηση"** <img src='data:image/png;base64,' />.