---
title: '8000020: Τρόπος εξαγωγής επαφών'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000020"
- "2003"
ms.assetid: ''
ms.openlocfilehash: d7dfdf30ec51f393cb47cb3fe89577923d4ac054579391f1fa9700bb9c3c6319
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: el-GR
ms.lasthandoff: 08/05/2021
ms.locfileid: "53983273"
---
# <a name="export-contacts"></a>Εξαγωγή επαφών

1. Στο κάτω μέρος της σελίδας στο Outlook στο web, επιλέξτε το κουμπί <img src='data:image/png;base64,' />
 **Άτομα.**
2. Στη γραμμή εργαλείων, επιλέξτε **"Διαχείριση**  >  **επαφών εξαγωγής".** 
3. Επιλέξτε να εξαγάγετε όλες τις επαφές ή μόνο τις επαφές από έναν συγκεκριμένο φάκελο και, στη συνέχεια, επιλέξτε **"Εξαγωγή".**
4. Στο κάτω μέρος της σελίδας, επιλέξτε **"Αποθήκευση"** για να αποθηκεύσετε "contacts.csv" στον προεπιλεγμένο φάκελο "Λήψεις". Εναλλακτικά, επιλέξτε το βέλος δίπλα στην επιλογή **"Αποθήκευση"** και επιλέξτε **"Αποθήκευση ως".**

Για να μάθετε περισσότερα, ανατρέξτε [στο θέμα Εξαγωγή επαφών από Outlook.](https://support.office.com/article/10f09abd-643c-4495-bb80-543714eca73f#ID0EAACAAA=Outlook_on_the_web)

